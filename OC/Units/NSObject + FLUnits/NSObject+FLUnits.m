...
@interface _FLBlockTarget : NSObject

/**添加一个KVOBlock*/
- (void)fl_addBlock:(void(^)(__weak id obj, id oldValue, id newValue))block;
- (void)fl_addNotificationBlock:(void(^)(NSNotification *notification))block;

- (void)fl_doNotification:(NSNotification*)notification;

@end

@implementation _FLBlockTarget{
    //保存所有的block
...
@implementation NSObject (FLUnits)

static void *const FLKVOBlockKey = "FLKVOBlockKey";
static void *const FLKVOSemaphoreKey = "FLKVOSemaphoreKey";
...
+ (BOOL)fl_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
    return [GetClass((id)self) fl_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}


- (void)fl_addObserverBlockForKeyPath:(NSString*)keyPath block:(void (^)(id obj, id oldVal, id newVal))block {
    if (!keyPath || !block) return;
    dispatch_semaphore_t kvoSemaphore = [self _fl_getSemaphoreWithKey:FLKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, FLKVOBlockKey);
    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, FLKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    _FLBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [_FLBlockTarget new];
        //保存
    ...
    [targetForKeyPath fl_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self _fl_swizzleDealloc];
...
- (void)fl_removeObserverBlockForKeyPath:(NSString *)keyPath{
    ...
    _FLBlockTarget *target = allTargets[keyPath];
    if (!target) return;
    dispatch_semaphore_t kvoSemaphore = [self _fl_getSemaphoreWithKey:FLKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
...
- (void)fl_removeAllObserverBlocks {
    ...
    dispatch_semaphore_t kvoSemaphore = [self _fl_getSemaphoreWithKey:FLKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, _FLBlockTarget *target, BOOL *stop) {
...
static void *const FLNotificationBlockKey = "FLNotificationBlockKey";
static void *const FLNotificationSemaphoreKey = "FLNotificationSemaphoreKey";

- (void)fl_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notification))block {
    if (!name || !block) return;
    dispatch_semaphore_t notificationSemaphore = [self _fl_getSemaphoreWithKey:FLNotificationSemaphoreKey];
    dispatch_semaphore_wait(notificationSemaphore, DISPATCH_TIME_FOREVER);
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, FLNotificationBlockKey);
    if (!allTargets) {
        allTargets = @{}.mutableCopy;
        objc_setAssociatedObject(self, FLNotificationBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _FLBlockTarget *target = allTargets[name];
    if (!target) {
        target = [_FLBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(fl_doNotification:) name:name object:nil];
    }
    [target fl_addNotificationBlock:block];
    [self _fl_swizzleDealloc];
...
- (void)fl_removeAllNotification{
    ...
    dispatch_semaphore_t notificationSemaphore = [self _fl_getSemaphoreWithKey:FLNotificationSemaphoreKey];
    dispatch_semaphore_wait(notificationSemaphore, DISPATCH_TIME_FOREVER);
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, _FLBlockTarget *target, BOOL * _Nonnull stop) {
        [[NSNotificationCenter defaultCenter] removeObserver:target];
    }];
    [allTargets removeAllObjects];
    dispatch_semaphore_signal(notificationSemaphore);
...
- (void)fl_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo{
...
- (void)_fl_swizzleDealloc{
    ...
    @synchronized(swizzleClass) {
        ...
        id newDealloc = ^(__unsafe_unretained id objSelf){
            //在这里我们移除所有的KVO
            [objSelf fl_removeAllObserverBlocks];
            //移除所有通知
            [objSelf fl_removeAllNotification];
...
- (dispatch_semaphore_t)_fl_getSemaphoreWithKey:(void *)key{
