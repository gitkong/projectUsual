...
#pragma mark - 正常情况下的左对齐
- (void)normalAligmetnLeft{
    //    获得按钮的图片的frame
    ...
    //    设置按钮的文本的x坐标为0-－－左对齐
    titleFrame.origin.x = CGRectGetMaxX(imageFrame);
...
- (void)layoutSubviews{
    ...
    // 判断
    if (_status == FLAlignmentStatusNormalCenter) {
        
    }
    else if (_status == FLAlignmentStatusNormalLeft){
        [self normalAligmetnLeft];
    }
