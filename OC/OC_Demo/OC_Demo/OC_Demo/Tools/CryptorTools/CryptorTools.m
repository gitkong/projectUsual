...
+ (NSString *)AESEncryptString:(NSString *)string keyString:(NSString *)keyString iv:(NSData *)iv {
    ...
    // BASE 64 编码
    NSString *base64Str = [result base64EncodedStringWithOptions:0];
    // 将加密后的出现的+号替换成/add*/
//    return [base64Str stringByReplacingOccurrencesOfString:@"+" withString:@"/add*/"];
    return base64Str;
}
...
+ (NSString *)AESDecryptString:(NSString *)string keyString:(NSString *)keyString iv:(NSData *)iv {
    // BASE 64 解码
//    NSData *data = [[NSData alloc] initWithBase64EncodedString:[string stringByReplacingOccurrencesOfString:@"/add*/" withString:@"+"] options:0];
    NSData *data = [[NSData alloc] initWithBase64EncodedString:string options:0];
    NSData *result = [self AESDecryptData:data keyString:keyString iv:iv];
    NSString *decodeStr = [[NSString alloc] initWithData:result encoding:NSUTF8StringEncoding];
    return decodeStr;
}
