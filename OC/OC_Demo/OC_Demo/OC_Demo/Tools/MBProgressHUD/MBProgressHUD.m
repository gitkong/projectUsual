...
//								constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode];
    CGRect detailsLabelRect = [detailsLabel.text boundingRectWithSize:maxSize options:0 attributes:@{NSFontAttributeName : detailsLabel.font} context:nil];
                               
	totalSize.width = MAX(totalSize.width, detailsLabelRect.size.width);
	totalSize.height += detailsLabelRect.size.height;
	if (detailsLabelRect.size.height > 0.f && (indicatorF.size.height > 0.f || labelSize.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	detailsLabelF.origin.x = roundf((bounds.size.width - detailsLabelRect.size.width) / 2) + xPos;
	detailsLabelF.size = detailsLabelRect.size;
